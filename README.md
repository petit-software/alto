...
[diagnosis and regression checks](docs/audio-noise-investigation.md).
...
the player. **Settings → Reading → Player size** offers Default, 1.25× and 1.5×.
...
There is no separate sidebar window. **Models…**, **Read Text…**, and **Finish
Setup…** open the appropriate Settings tab; the floating player stays separate.
...
recommendation, not a measured minimum. See [verification](docs/verification.md)
...
See [docs/verification.md](docs/verification.md) for completed checks and manual
